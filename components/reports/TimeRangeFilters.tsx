    ...
    return (
        <div className="bg-card border rounded-lg p-6 space-y-7">
            ...
            <div className="flex flex-col md:flex-row md:items-end gap-6">
                ...
                <button className="w-fit min-w-52 flex flex-row justify-center items-center py-2.5 md:py-2 bg-indigo-100/50 space-x-5 h-fit text-sm font-semibold group hover:bg-indigo-500 hover:text-white rounded-sm" onClick={() => updateFilters("", "")}><RefreshCcw className="w-4 h-4 mr-2 text-indigo-500 group-hover:text-white " /> Atur Ulang Tanggal</button>
            </div>
            
            <div className="flex flex-wrap gap-3">
                <button className="hover:bg-primary hover:text-white p-2 rounded-sm text-black bg-gray-100" onClick={() => setQuickRange(7)}>7 Hari Terakhir</button>
                <button className="hover:bg-primary hover:text-white p-2 rounded-sm text-black bg-gray-100" onClick={() => setQuickRange(30)}>30 Hari Terakhir</button>
                <button className="hover:bg-primary hover:text-white p-2 rounded-sm text-black bg-gray-100" onClick={() => setThisMonth()}>Bulan Ini</button>
            </div>
        </div>
    );
}
